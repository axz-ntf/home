...
import { nearbyStations } from "./subway";
...
// 캐시 키 규칙 — route / warm 스크립트가 반드시 동일해야 함.
export function insightCacheKey(lat: number, lng: number, type?: HousingTypeId | string | null): string {
  return `${lat.toFixed(4)},${lng.toFixed(4)},${type ?? ""}`;
}
